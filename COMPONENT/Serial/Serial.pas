unit Serial;
{********************************************************************
  TSerialPort component for Delphi
...
type
  ...
  public
    ...
    procedure   FlushTX;
    procedure   FlushRX;
    function    TransmitStatus : boolean;
    function    ReceiveStatus  : boolean;
  published
...
var
	...
	// ReadFile operation.
	i, j	       : LongInt;		// Recieve Data Index
...
begin
	...
	ReadBuffer := '';		//	ReadBuffer clear(You Must clear read area buffer)

	if assigned(fOnReceive) then
		fONReceive(self, ReadBuffer);

// Get the total number of bytes that are waiting to be read from the input buffer.
	ClearCommError(hCommPort, dwErrorCode, @oStatus);
	BytesInQueue := oStatus.cbInQue;

	if BytesInQueue > 0 then begin
		SetLength(ReadBuffer, BytesInQueue);
		ReadFile(hCommPort,
				 PChar(ReadBuffer)^,
				 BytesInQueue,
				 NumBytesRead,
				 nil);
		// Recieve Data 중에서 null 문자 제거
		j := 1;
		for i := 1 to BytesInQueue do begin
			if ReadBuffer[i] <> #0 then begin
...
begin
  if fParityErrorChecking <> value then begin
...
begin
  if fParityErrorChar <> value then begin
...
procedure Register;
begin
  RegisterComponents('uniHISPACK', [TSerialPort]);
end;

function TSerialPort.TransmitStatus : boolean;
begin
...
function TSerialPort.ReceiveStatus : boolean;
begin
	if hCommPort = INVALID_HANDLE_VALUE then begin
   	result := false;
   	exit;
