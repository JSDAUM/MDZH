...
begin
  ...
  while FHTMLPopupWindow.Top < GetSystemMetrics(SM_CYFULLSCREEN) do
  begin
    t := GetTickCount;
...
var
  t: DWORD;
begin
  FTop := GetSystemMetrics(SM_CYFULLSCREEN);
  FLeft := GetSystemMetrics(SM_CXFULLSCREEN) - FWidth - 20;
  Show;

  while FHTMLPopupWindow.Top + FHTMLPopupWindow.Height > GetSystemMetrics(SM_CYFULLSCREEN) do
  begin
    t := GetTickCount;
    while GetTickCount - t < 50 do
      Application.ProcessMessages;

    if FHTMLPopupWindow.Top - 20 < GetSystemMetrics(SM_CYFULLSCREEN) - FHTMLPopupWindow.Height then
      FHTMLPopupWindow.Top := GetSystemMetrics(SM_CYFULLSCREEN) - FHTMLPopupWindow.Height
    else
      FHTMLPopupWindow.Top := FHTMLPopupWindow.Top - 20;
  end;
end;
