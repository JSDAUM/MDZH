...
type
  ...
  TSortStyle = (ssAutomatic, ssAlphabetic, ssNumeric, ssDate, ssAlphaNoCase,
    ...
    ssAnsiAlphaNoCase, ssRaw, ssHTML, ssImages);
  ...
  TRawCompareEvent = procedure(Sender:TObject; Col,Row1,Row2: Integer;
    var Res: Integer) of object;
  ...
  TGridAlignEvent = procedure (Sender: TObject; ARow, ACol: Integer;
    var HAlign: TAlignment;var VAlign: TVAlignment) of object;
  ...
  TCellValidateEvent = procedure(Sender: TObject; Col, Row: Integer;
  ...
  TCellType = (ctBitmap,ctIcon,ctNone,ctImageList,ctCheckBox,ctDataCheckBox,
    ...
    ctValue,ctProgress,ctComment,ctButton,ctBitButton);
  ...
  published
    ...
    property DisjunctRowSelect: Boolean read FDisjunctRowSelect write FDisjunctRowSelect default False;
    property DisjunctCellSelect: Boolean read FDisjunctCellSelect write FDisjunctCellSelect default False;
  ...
  published
    ...
    property AllowClipboardRowGrow: Boolean read FAllowClipboardRowGrow write FAllowClipboardRowGrow default False;
    property AllowClipboardColGrow: Boolean read FAllowClipboardColGrow write FAllowClipboardColGrow default False;
    ...
    property CopyHTMLTagsToClipboard: Boolean read FCopyHTMLTagsToClipboard write FCopyHTMLTagsToClipboard default False;
  ...
  private
    ...
    procedure SetCurrentCell(const AValue:string);
...
const
  ...
  {$IFNDEF DELPHI3_LVL}
  //defined in Delphi 3+ and BCB3+ but not in Delphi 2, BCB1
...
begin
  if (Key in [VK_ESCAPE,VK_PRIOR,VK_NEXT,VK_END,VK_RETURN]) then
  begin
    if (Key = VK_ESCAPE) then
      try
...
begin
  ...
  edFloatEditBtn:begin
           if Msg.CharCode in Float_Characters then
             begin
             if ((Msg.CharCode=ord('.')) or
                 (Msg.CharCode=ord(','))) and
                 (( (pos('.',self.Text)>0) and (pos('.',self.SelText)=0) ) or
                  ((pos(',',self.Text)>0) and (pos(',',self.SelText)=0))) then Exit;
    ...
    begin
      ...
      begin
        if ((msg.CharCode = ord('.')) or
           (msg.CharCode = ord(','))) and
           (( (pos('.',self.Text) > 0) and (pos('.',self.SelText)=0) ) or
            ((pos(',',self.Text) > 0) and (pos(',',self.SelText)=0))) then
          Exit;
...
begin
  if Assigned(OnCanEditCell) then
    OnCanEditCell(Self,ARow,ACol,IsReadOnly);
end;
...
begin
  BC := BaseCell(ACol,ARow);
  Result := (CellTypes[BC.X,BC.Y] in [ctCheckBox,ctDataCheckBox]);
end;
...
begin
  BC := BaseCell(ACol,ARow);
  Result := (CellTypes[BC.X,BC.Y] = ctDataCheckBox);
end;
...
begin
  BC := BaseCell(ACol,ARow);
  Result := (CellTypes[BC.X,BC.Y] in [ctCheckBox,ctDataCheckBox,ctRadio]);
end;
...
begin
  Result := False;
  if not (goEditing in Options) and not FEditDisable then
    Exit;
...
begin
  ...
  begin
    ...
    begin
      EditMode := False;
      Cells[ACol,ARow] := EditCombo.Text;
      EditCombo.Visible := False;
      EditCombo.Enabled := False;
    end;
  end;
  ...
  begin
    ...
    if ComCtrlOk then
    begin
      Cells[ACol,ARow] := DateToStr(EditDate.Date);
      EditDate.Enabled := False;
      EditDate.Visible := False;
      EditMode := False;
    end;
  ...
  begin
    ...
    if ComCtrlOk then
    begin
      Cells[ACol,ARow] := TimeToStr(EditDate.time);
      EditDate.Enabled := False;
      EditDate.Visible := False;
      EditMode := False;
    end;
  ...
  edUnitEditBtn:
  begin
    Cells[ACol,ARow] := UnitEditBtn.Text + UnitEditBtn.UnitID;
    UnitEditBtn.Enabled := False;
    UnitEditBtn.Visible := False;
    EditMode := False;
  end;
  edRichEdit:
  begin
    RichToCell(ACol,ARow,FInplaceRichEdit);
    ...
    begin
      ...
      edSpinEdit:
        begin
          ...
          EditSpin.Value := Ints[ACol,ARow];
        end;
      edFloatSpinEdit:
        begin
          EditSpin.SpinType := sptFloat;
          EditSpin.FloatValue := Floats[ACol,ARow];
        end;
      edTimeSpinEdit:
        begin
          EditSpin.SpinType := sptTime;
          EditSpin.Text := Cells[ACol,ARow];
        end;
      edDateSpinEdit:
        begin
          EditSpin.dateValue := Dates[OCol,ARow];
...
begin
  ...
  if (Result=0) and FixedColAlways then Result := 1;
end;
...
begin
  ...
  begin
    r := CellRect(ACol,ARow);
    Result.cy := DrawText(Canvas.Handle,PChar(s),Length(s),r,DT_CALCRECT or DT_WORDBREAK or DT_LEFT) + 2 * FXYOffset.Y;
  ...
  begin
    ...
    Result.cx := MaxSize;
    Result.cy := NumLines * Canvas.TextHeight('gh') + 2 * FXYOffset.Y;
  end;
...
begin
  ...
  if (((ACol >= Selection.Left) and (ACol <= Selection.Right) and
     ...
     (MouseActions.DisjunctRowSelect and RowSelect[ARow])  then
  begin
...
begin
  ...
  if not Print and Select and ShowSelection and not FixedCell and
    (IsSelected(ACol,ARow) and not MouseActions.DisjunctRowSelect)
...
var
  ...
  begin
    ...
    if Assigned(OnGetCellBorderProp) then
      OnGetCellBorderProp(Self, ACol, ARow, PenL, PenT, PenR, PenB);
  ...
  begin
    ...
    if (ACol >= FixedCols) and (ARow >= FixedRows) and not FMouseDown then
      if FMouseActions.DisjunctRowSelect and not RowSelect[ARow] then
      begin
        URLCol := FURLColor;
    ...
    begin
      ...
      HTMLDrawEx(Canvas,TmpStr,Rect,Gridimages,
               Rect.Left,Rect.Top,-1,0,1,False,False,False,False,FGridBlink,False,not EnhTextSize,False,
    ...
    if FSortSettings.Show and (ARow = FSortSettings.Row) and (RowCount > 2) and
      ...
      begin
        ...
        case AlignValue of
        taLeftJustify:Rect.Left := Rect.Left + SortWidth + 10;
...
var
  ...
  rc: Integer;
begin
  if FMouseActions.RangeSelectAndEdit then
  begin
    Options := Options + [goEditing];
    r := CellRect(Col,Row);
...
begin
  ...
  begin
    if Col + CellSpan(Col,Row).X + 1 < ColCount - FixedRightCols + NumHiddenColumns then
...
begin
  ...
  begin
    ...
    begin
      ...
      if (RowCount - FixedRows - FixedFooters = 1) then
        ClearRows(Row,1)
      else
        RemoveRows(Row,1);

      if Assigned(FOnAutoDeleteRow) then
        FOnAutoDeleteRow(self,CurRow);
  ...
  begin
    ...
    if (FixedRightCols > 0) and (SelCol = ColCount - FixedRightCols - 1) then
    begin
  ...
  begin
    ...
    begin
      ...
      if not HTMLDrawEx(Canvas,s,r,Gridimages,Msg.XPos,Msg.YPos,-1,0,1,
                        ...
                        0.0,FURLColor,clNone,clNone,clGray,Anchor,Stripped,FocusAnchor,
                        XSize,YSize,ml,hl,hr,cr,CID,CT,CV,FImageCache,FContainer,self.Handle) then
        Anchor := '';
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        GridCells[X,Y] := FileToLF(ss,FMultiLineCells);
      end;
...
var
  i,j,mc,mr: Integer;
  s,al,ac,afs,afe,ass,ase,tablestyle: string;
  slist: TStringlist;
  AlignValue: TAlignment;
  TopRow: Integer;
  strtCol,strtRow: Integer;
    ...
    begin
      ...
      begin
       ...
       begin
        ...
        Add('<td' + wraptxt + al + ac + colwtxt + spantxt + '><b>' + s + '</b></td>');
...
begin
  ...
  begin
    if not (MouseActions.DisjunctRowSelect) or RowSelect[z]  or not DoDisjunct then
    begin
...
begin
  ...
  if not ((goEditing in Options) or
    (Navigation.AllowClipboardAlways)) then
     Exit;
  ...
  begin
    ...
    begin
      ...
      if not (VarType(fCell) in [varEmpty,varDispatch,varError]) then
        Cells[z - 1 + StrtCol,s - 1 + StrtRow] := fCell;
    end;
  end;
...
begin
  ...
  begin
    for j := 1 to i do
      Cells[FixedCols + j - 1, k] := ObjRSC.Fields[j - 1].Value;
    ObjRSC.MoveNext;
    inc(k);
    if k > RowCount then
      RowCount := RowCount + 1;
...
begin
  ...
  begin
    ...
    begin
      ...
      ss1 := GetToken(ss,',');
      Y := StrToInt(ss1);
      Cells[X+SaveStartCol,Y+SaveStartRow] := FileToLF(ss,FMultiLineCells);
    end;
  end;
...
begin
  if RowCount > 0 then
  for r := FixedRows to RowCount -1 - FFixedFooters do
    Ints[ACol,r] := r - FixedRows + 1;
end;
...
begin
  if ColCount > 0 then
  for c := FixedCols to ColCount -1 - FFixedRightCols do
    Ints[c,ARow] := c - FixedCols + 1;
end;
...
begin
  ...
  // Allow 2 pixel spacing at begin & end
  NewH := MaxHeight + XYOffset.Y + 2 * (GridLineWidth + 1);
...
begin
  ...
  for i := j to RowCount-1 do
    ...
    begin
      AutoSizeRow(i);
      if Padding <> 0 then
        RowHeights[i] := RowHeights[i] + Padding;
    end
    else
...
begin
  th := Canvas.TextHeight('gh');
  RowHeights[ARow] := CheckLimits(Padding+((th + (th shr 2))*Lines),MinRowHeight,MaxRowHeight);
end;
...
var
  ...
  var
    ...
    CID,CV,CT: string;
  begin
    if HasDataCheckBox(Col,Row) then
      s := ''
    else
      s := Cells[Col,Row];
 ...
 {$ENDIF}
  ...
  begin
    ...
    begin
      ...
      if HasDataCheckBox(cn,ARow) then
        cs := ''
      else
        cs := Cells[cn,ARow];
      ...
      begin
        ...
        if (HAlign = taRightJustify) and
           (cg.CellType in [ctCheckbox, ctDataCheckbox]) and (s = '')  then
        begin
          rg.Right := tr.Right;
          rg.Left := rg.Right - CGS.X;
          tr.Right := rg.Left;
   ...
   begin
     ...
     begin
       HTMLRect := Rect(Indent, 0, xsize, -FPrintSettings.FHeaderSize);
      ...
      begin
        ...
        begin
          if Assigned(FOnPrintPageDone) then
            FOnPrintPageDone(Self, Canvas, i, YPosPrint, PagNum, XSize, YSize);
        end;
      end;
...
begin
  MouseToCell(x,y,c,r);
  if MouseActions.DisjunctCellSelect and (c > -1) and (r > -1) then
  begin
    c := RealColIndex(c);
...
begin
{
  ...
  begin
    if ARow > FMouseSelectStart then
      Selection := TGridRect(Rect(FixedCols,FMouseSelectStart,ColCount - 1, ARow))
    else
      Selection := TGridRect(Rect(FixedCols,ARow,ColCount - 1, FMouseSelectStart));
  end;
  ...
  begin
    if ACol > FMouseSelectStart then
      Selection := TGridRect(Rect(FMouseSelectStart,FixedRows,ACol,RowCount - 1))
    else
      Selection := TGridRect(Rect(ACol,FixedRows,FMouseSelectStart,RowCount - 1));
  end;
...
begin
  ...
  begin
    prevcurs := SetCursor(LoadCursor(hinstance,makeintresource(8000)));
    wheelpan := True;
...
begin
  FBlockFocus := True;
  HideInplaceEdit;
  if Key in [VK_DOWN,VK_UP,VK_LEFT,VK_RIGHT,VK_TAB] then Keydown(Key,shift);
  FOldAlwaysEdit := FNavigation.AlwaysEdit;
  FNavigation.AlwaysEdit := False;
  SetFocus;
  if Key = VK_RETURN then
...
begin
  vn := GetVersionNr;
  Result := IntToStr(Hi(Hiword(vn)))+'.'+IntToStr(Lo(Hiword(vn)))+'.'+IntToStr(Hi(Loword(vn)))+'.'+IntToStr(Lo(Loword(vn)))+' May, 2002';
end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            FGrid.RowHeights[FGrid.Row] := NewHeight;
          end;
        end;
...
begin
  ...
  Canvas.Pen.Color :=  FGrid.FixedColor;
  Canvas.Rectangle(R.Left,R.Top,R.Right,R.Bottom);
