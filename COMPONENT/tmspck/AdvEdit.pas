...
begin
  ...
  if not (IsError and ShowError) then
  begin
    Color := FNormalColor;
  ...
  if Assigned(FOnValueValidate) then
    FOnValueValidate(Self,Self.Text,IsValid);
...
begin
 ...
 if fShowModified then
  begin
   if (value=false) then self.Font.Color:=fFontColor
                    else self.Font.Color:=fModifiedColor;
  end;
...
begin
  ...
  if (Msg.CharCode = VK_RETURN) and FDefaultHandling then
  begin
    // Take care of default key handling
...
begin
 if fFlat<>value then
  begin
   ...
   if fFlat then
    begin
     self.color:=(parent as twincontrol).brush.color;
     self.borderstyle:=bsNone;
     loc.left:=0;
     loc.top:=0;
     loc.right:=clientrect.right;
     loc.bottom:=clientrect.bottom-1;
...
begin
 ...
 if fFlat then
  begin
   ...
   oldpen:=selectobject(dc,createpen( PS_SOLID,1,rgb(128,128,128)));
   movetoex(dc,0,self.height-1,nil);
   lineto(dc,self.width,self.height-1);
   deleteobject(selectobject(dc,oldpen));
...
begin
 if fShowModified then
  begin
   if value=false then self.Font.Color:=fFontColor else self.Font.Color:=fModifiedColor;
  end;
 inherited Modified:=value;
end;
...
begin
  ...
  if (FAdvEdit.fIsDragSource) then
   begin
    ...
    if (uchar>=fAdvEdit.SelStart) and (uchar<=fAdvEdit.SelStart+fAdvEdit.SelLength) then exit;
   end;
