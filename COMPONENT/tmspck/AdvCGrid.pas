...
{ version 2.2.0.6 - rel. April, 2002                                       }
{                                                                          }
...
begin
  ...
  begin
    ...
    begin
      Result := TGridColumnItem(FColumnCollection.Items[Col]).EditLength;
    end;
  end;
end;
...
begin
  ...
  begin
    if ACol >= FixedCols then
    begin
      Result := TGridColumnItem(FColumnCollection.Items[ACol]).EditMask;
    end;
  end;
...
begin
  vn := GetVersionNr;
  Result := IntToStr(Hi(Hiword(vn)))+'.'+IntToStr(Lo(Hiword(vn)))+'.'+IntToStr(Hi(Loword(vn)))+'.'+IntToStr(Lo(Loword(vn)))+' April, 2002';
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FCellGraphic.CellType := ctDataCheckBox;
...
begin
  ...
  if (R = 0) and (C <> FFilterCol) then
  begin

    CR := CellRect(C,R);

    if (X > CR.Right - 16) and
       (X < CR.Right - 3) and (FColumnCollection.Items[C].Filter.Count > 0) then
    begin
      FFilterCol := C;
      ...
      for I := 1 to FColumnCollection.Items[C].Filter.Count do
      begin

        VP := Pos('=', FColumnCollection.Items[C].Filter.Strings[I - 1]);
        if VP > 0 then
          FDropList.Items.Add(Copy(FColumnCollection.Items[C].Filter.Strings[I - 1],1,VP - 1))
        else
          FDropList.Items.Add(FColumnCollection.Items[C].Filter.Strings[I - 1]);
        if I = 1 then
