...
uses
  stdctrls,graphics,wintypes,winprocs,messages,classes,controls,sysutils;
...
type
 ...
 TEditListBox = class(TListBox)
  private
    ...
    { unused, replaced by FlatSetScrollInfo
    procedure FlatSetScrollRange(code,nMinValue,nMaxValue:integer;fRedraw:bool);
    }
...
var
 ...
 r:trect;
begin
 if (msg.TimerID=fListTimerID)
     and not fStopHover and
...
{ unused, replaced by FlatSetScrollInfo
procedure TEditListBox.FlatSetScrollRange(code, nMinValue,
  nMaxValue: integer; fRedraw: bool);
var
 ComCtl32DLL: THandle;
 _FlatSB_SetScrollRange:function(wnd:hwnd;code,nMinPos,nMaxPos:integer;fRedraw:bool):integer; stdcall;
begin
 ComCtl32DLL:=GetModuleHandle(commctrl);
 if (ComCtl32DLL>=32) then
  begin
   @_FlatSB_SetScrollRange:=GetProcAddress(ComCtl32DLL,'FlatSB_SetScrollRange');
   if assigned(_FlatSB_SetScrollRange) then
     _FlatSB_SetScrollRange(self.handle,code,nMinValue,nMaxValue,fRedraw);
  end;
end;
}
