...
{ version 2.2.0.5 - rel. April 2002                                         }
{                                                                           }
...
begin
  ...
  begin
    ...
    Canvas.LineTo(ARect.Right - GLW ,ARect.Bottom - GLW);
  end;
...
begin
  ...
  if HasCellProperties(ACol,ARow) then
    ...
    begin
      ...
      //Result := Point(CellProperties[ACol,ARow].CellSpanX,CellProperties[ACol,ARow].CellSpanY);
    end;
end;
...
begin
  ...
  if Assigned(OnGetDisplText) and (BC.Y < FNormalRowCount) then
    OnGetDisplText(Self,BC.X,BC.Y,Result)
end;
