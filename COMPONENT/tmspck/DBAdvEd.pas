...
type
  ...
  published
    ...
    property ClearOnInsert: Boolean read FClearOnInsert write FClearOnInsert default True;    
...
begin
  ...
  begin
    ...
    etNumeric:
      begin
        if (FDataLink.Field.AsString = '') then self.IntValue := 0
        else self.IntValue := FDataLink.Field.AsInteger;
      end;
    else
  ...
  if (FDataLink.DataSet.State = dsInsert) and FClearOnInsert then
  begin
