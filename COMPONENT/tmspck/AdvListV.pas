...
type
  ...
  protected
    ...
    procedure MeasureItem(var Height: Integer); virtual;
  ...
  published
    ...
    property FilterBar: Boolean read FFilterBar write ShowFilter default False;
...
var
 lis:tlistitem;
begin
 if (c>=self.columns.Count) then Exit;
 if (r>=self.Items.Count) then Exit;
...
procedure TAdvListView.MeasureItem(var Height: Integer);
begin
  Height := 48;
end;
...
begin
  ItemToRich(itemindex,subitemindex,FRichEdit);
...
begin
       ...
       begin
         ...
         begin
           ...
           begin
             case VAlignment of
             vtaTop:ih := Rect.top;
             vtaCenter:ih := Rect.top+((rect.bottom-rect.top)-Smallimages.Height) shr 1;
 ...
 if Assigned(FOnDrawItem) then FOnDrawItem(Self,FCanvas,Index,OrgRect,State);
...
begin
  ...
  begin
    ...
    SetTextAtColRow(FColumnIndex,FItemIndex,StrPas(buf));
    if (FColumnIndex>0) then
...
begin
  ...
  begin
    ...
    if odFocused in State then
     begin
       if RowSelect then DrawFocusRect(hDC,rcItem)
       else
        begin
          rcItem.Right := self.Columns[0].width;
...
procedure TAdvListView.WMDrawItem(var Message: TWMDrawItem);
begin
  DefaultHandler(Message);
end;

procedure TAdvListView.WMMeasureItem(var Message: TWMMeasureItem);
begin
  DefaultHandler(Message);
end;
...
begin
  ...
  begin
    ...
    with PNMListView(Pointer(Message.NMHdr))^ do
    begin
      if (iItem - 1 = SortColumn) and (SortColumn <> -2) and SortShow then
      begin
        if SortUpGlyph.Empty or SortDownGlyph.Empty then
        begin
          case SortDirection of
          sdAscending:ColumnImages[iItem] := 0;
          sdDescending:ColumnImages[iItem] := 1;
          end
        end
        else
        begin
        ...
        if Sortshow then ColumnImages[iItem] := -1;
...
begin
  ...
  begin
    hHeader := SendMessage(self.Handle, LVM_GETHEADER, 0, 0);
    SendMessage(hHeader, WM_SETFONT, FHeaderFont.Handle,makelparam(WORD(TRUE),0));
...
begin
  ...
  begin
    ...
    if (Sender=HeaderImages) then SetHeaderList(ImageHandle,TVSIL_NORMAL)
  end;
end;
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          s := MakeHREF(s);
          AlignValue:=self.columns[j+1].Alignment;
...
begin
 ...
 hHeader := SendMessage(self.Handle, LVM_GETHEADER, 0, 0);
  ...
  if onoff then rstyle := rstyle or alvHDS_FILTERBAR else
...
var
  ...
  FHeaderHandle: THandle;
begin
  if (AColumn >= Columns.Count) or (AColumn < 0) then Exit;

  if SortShow and
     (SortUpGlyph.Empty = False) and
     (SortDownGlyph.Empty = False) then
  begin
     ...
     begin
      ...
      begin
        if (pos(#13,s) > 0) then
          ...
          DrawTextEx(Canvas.Handle,pchar(s),length(s),rcItem,ali or DT_VCENTER or DT_SINGLELINE or DT_END_ELLIPSIS,nil);
...
begin
  ...
  {$R-}
  SendMessage(hHeader, WM_SETFONT, DWORD( FDummyFont.Handle ),makelparam(WORD(TRUE),0));
  {$R+}
...
begin
  ...
  hHeader := SendMessage(self.Handle, LVM_GETHEADER, 0, 0);
  Header_SetImageList(hHeader,HeaderImages.Handle,LVSIL_SMALL);
end;
...
begin
  if FListTimerID <> -1 then KillTimer(Handle,TimerID);
  FListTimerId := -1;
  FRichEdit.Free;
...
begin
  ...
  if not (Parent as TWinControl).HandleAllocated then Exit;
...
begin
  ...
  begin
    ...
    if FSortShow then
    begin
      if (SortUpGlyph.Empty or SortDownGlyph.Empty)
         and Assigned(Headerimages) then
      begin
        if SortDirection = sdAscending then
          ColumnImages[FOldSortCol] := 0
        else
          ColumnImages[FOldSortCol] := 1;
      end;

      if not SortUpGlyph.Empty and not SortDownGlyph.Empty then
      begin
...
begin
 ...
 { colchgflg:=false; }
  ...
  if w < self.width then
    Columns[Columns.Count - 1].Width := ClientRect.Right - w - 6;
end;
...
begin
  ...
  if (aIdx2=-1) then
  begin
    (self.items.Add as TListItem).Assign(listitem);
...
begin
  ...
  begin
    ...
    bmp.width:=width;
    bmp.height:=height;
    bmp.Canvas.StretchDraw(clienTRect, FWallPaper);
    }
