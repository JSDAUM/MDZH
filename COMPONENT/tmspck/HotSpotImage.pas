...
type
  ...
  protected
    ...
    procedure DrawHoverImage(HotSpotIndex: Integer);
    procedure DrawClickImage(HotSpotIndex: Integer);
...
var
  source, ttemp, tempmask, tempim: TBitmap;
  timlist: TImageList; //for drawing images with a mask
begin
  if EmptyImage(Pict) then
  begin
    Bitm.Height := 0;
    Bitm.Width := 0;
    Exit;
  ...
  if Assigned(FPicture) and (FClipped) and (not EmptyImage(FPicture)) then
  begin
    // Create the mask which will select what part of the clicked image
    // will be really drawn
    tempmask := TBitmap.Create;
    tempmask.PixelFormat := pf24bit;

    // Create the temporary bitmaps
    tempim := TBitmap.Create;
    ttemp := TBitmap.Create;
    ttemp.PixelFormat := pf24bit;

    // The background image from which we will copy the background
    source := TBitmap.Create;
    source.PixelFormat := pf24bit;
    timlist := TImageList.CreateSize(min(Width,Pict.Width),min(Height,Pict.Height));

    try
      // Initialize and draw the mask into TempMask
      tempmask.Width := min(Width,Pict.Width);
      tempmask.Height := min(Height,Pict.Height);
      tempmask.Canvas.Brush.Style := bsSolid;
      tempmask.Canvas.Brush.Color := clblack;
      tempmask.Canvas.Pen.Color := clblack;
      tempmask.Canvas.Pen.Style := psSolid;
      DrawShapeAt(tempmask.Canvas,0,0);

      // Put click image into TempIm
      tempim.Assign(Pict.Graphic);
      tempim.Height := min(Height,Pict.Height);
      tempim.Width := min(Width,Pict.Width);

      // Add the click image to the list, using the mask
      timlist.Masked := True;
      timlist.bkColor := clNone;
      timlist.Add(tempim, tempmask);

      // Copy the background into ttemp
      Source.Width := FPicture.Width;
      Source.Height := FPicture.Height;
      Source.Canvas.Draw(0,0,FPicture.Graphic);
      ttemp.Width := Width;
      ttemp.Height := Height;
      ttemp.Canvas.CopyRect(Rect(0,0,ttemp.Width,ttemp.Height),Source.Canvas,AsRect);

      // Draw the clickimage over the background.
      // Because of the mask, only the right pats of the click image will be drawn
      timlist.Draw(ttemp.Canvas,0,0,0);

      // DebugShowBitmap(ttemp);

      //assigning the final image to the imgClick
      Bitm.Height := Height;
      Bitm.Width := Width;
      Bitm.Assign(ttemp);
  ...
  begin
    Bitm.Width:=min(Width,Pict.Width);
    Bitm.Height:=min(Height,Pict.Height);
...
//------------------------------------------------------------------------------
procedure THotSpotImage.DrawClickImage(HotSpotIndex: Integer);
begin
  ...
  with FHotSpots[HotSpotIndex] do
    ...
    begin
      CanVas.Draw(X,Y,FCCLick);
    end;
end;

//------------------------------------------------------------------------------
procedure THotSpotImage.DrawHoverImage(HotSpotIndex: Integer);
begin
  if (HotSpotIndex < 0) or (HotSpotIndex>= FHotSpots.Count) then
    Exit;
  with FHotSpots[HotSpotIndex] do
...
begin
  ...
  try
    ...
    begin
      ...
      Pen.Width := 1;
      Pen.Color := clBlack;
      Brush.Style := bsClear;// Modified to draw the shape
    ...
    begin
      ...
      FBitmap.Canvas.CopyRect(Rect(0,0,Width,Height), B.Canvas, Rect(0,0,Width,Height));

      //Draw the hoverimage
      if (FHoveredItem >= 0) and (FClickedItem = -1) then
      with FHotSpots[FHoveredItem] do
        if not EmptyImage(HoverImage) then
        begin
          Draw(FHotSpots[FHoveredItem].x, FHotSpots[FHoveredItem].y, FCHover);
        end;
      ...
      with FHotSpots[FClickedItem] do
        if not EmptyImage(ClickImage) then
        begin
          Draw(FHotSpots[FClickedItem].x, FHotSpots[FClickedItem].y, FCClick);
        end;

      for i := 1 to FHotSpots.Count do
      begin
        if FHotSpots[i - 1].Down then
        if not EmptyImage(FHotSpots[i - 1].ClickImage) then
        begin
          TC := TBitmap.Create;
          FHotSpots[i - 1].CalcClip(FHotSpots[i - 1].ClickImage,TC);
          Draw(FHotSpots[i - 1].x, FHotSpots[i - 1].y, TC );
          TC.Free;
        end;
      end;
...
begin
  ...
  begin
    ...
    try
      ...
      PC1.x := FOwner.Left;
      PC1.y := FOwner.Top;
      PC2.x := FOwner.Left + FOwner.ClientWidth;
      PC2.y := FOwner.Top + FOwner.ClientHeight;
      PM := ClientToScreen(PM);
...
begin
  ...
  try
    ...
    PC2.y := FOwner.Top+FOwner.ClientHeight;
    PM := ClientToScreen(PM);
    // PC1:=FOwner.ClientToScreen(PC1);
    // PC2:=FOwner.ClientToScreen(PC2);
    if (PM.x < PC1.x) or (PM.y < PC1.y) or (PM.x > PC2.x) or (PM.y > PC2.y) then
    begin
      HoveredItem := -1;
      Exit;
