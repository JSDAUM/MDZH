...
begin
  ...
  case FState of
    bsUp:
      if FMouseInControl then
        Frame3D(canvas, buttonRect, FColorHighlight, FColorShadow, 1)
      else
        Frame3D(canvas, buttonRect, FBorderColor, FBorderColor, 1);
    bsDown, bsExclusive:
      Frame3D(canvas, buttonRect, FColorShadow, FColorHighlight, 1);
    bsDisabled:
      Frame3D(canvas, buttonRect, FBorderColor, FBorderColor, 1);
  end;
